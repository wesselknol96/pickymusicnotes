...
function createNumberWheel({ value, min, max, step = 1, options = null, format = value => String(value), onChange }) {
    ...
    wheel.className = 'number-wheel vertical-number-wheel';
    wheel.tabIndex = 0;
    ...
    typed.addEventListener('input', () => {
        if (isNumberWheelDisabled(wheel)) return;
...
function stepNumberWheel(wheel, direction) {
    ...
    syncNumberWheelAnimationColor(wheel);
...
function isNumberWheelDisabled(wheel) {
    return wheel.getAttribute('aria-disabled') === 'true';
}
...
function syncNumberWheelAnimationColor(wheel) {
    const current = wheel.querySelector('.wheel-number.current');
    if (!current) return;
    const currentColor = getComputedStyle(current).color;
    const fallbackColor = getComputedStyle(wheel).color;
    wheel.style.setProperty(
        '--scroll-wheel-current-color',
        isTransparentCssColor(currentColor) ? fallbackColor : currentColor
    );
}
